...
def num_is_valid(x, seq):
    ...
    print(dir_options_d)
    print(seq + 'A')
    print(seq, (seq + 'A') in dir_options_d)
    return k == len(seq) and (c_i != 3 or c_j != 0) and ((seq + 'A') in dir_options_d)
...
def dir_options_global(seq): #seq should end with A
    global dir_options_d
    if seq in dir_options_d:
        return dir_options_d[seq]
    print(seq)
    os = []
    if len(seq) > 0:
...
def best_option_calc(seq):
    ...
    op_prev = dir_options_global(seq)
    op_new = set()
    for i in range(2):
        print(i, len(op_prev))
        for o in op_prev:
            ...
            for k in s[:-1]:
                ...
                # print('here', k, op_cur_a, opp)
                for a in op_cur_a:
                    op_cur_b.add(a + opp)
                op_cur_a = op_cur_b.copy()
                op_cur_b = set()
            op_new.update(op_cur_a)
        op_prev = op_new.copy()
        op_new = set()
    res = ''
    res_l = 1000000000000
    print('aaaa', len(op_prev))
    for o in op_prev:
        print(len(o))
        if len(o) < res_l:
            res = o
...
best_options = dict()
dir_options_d = {'A': 'A', '^A': '<A>A', '>A': 'vA^A', 'vA': '<vA>^A', '<A': 'v<<A>>^A',
                 '<<A': 'v<<AA>>^A', '>>A': 'vAA^A',
                 '>^A': 'vA<^A>A', '<vA': 'v^^A>A>^A',
                 '<^A': 'v<<A>^A>A', 'v>A': '<vA>A^A',
                 '>>^A': 'vAA<^A>A', 'v<<': '<vA<AA>>^A'}
cnt = 0
for line in lines:
    line = line.strip()
